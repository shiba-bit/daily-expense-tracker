...
async function handleOCRScan(e) {
    ...
    toggleSpinner(true);
    showToast("Initializing AI Scanner... Please wait.", "info");
    ...
    try {
        ...
        showToast("AI analyzing document...", "info");
        ...
        if (dateMatch) {
            // Very naive date conversion, fallback to today if parsing fails
            try {
        ...
        const totalRegex = /(?:total|amount|due|balance|pay)[\s.:]*\$?\s*(\d+(?:[.,]\d{2})?)/i;
        let amountFound = "0.00";
        const amtMatch = text.match(totalRegex);
        if (amtMatch && amtMatch[1]) {
            amountFound = parseFloat(amtMatch[1].replace(',', '.')).toFixed(2);
        } else {
            // Backup naive search for largest currency looking number
